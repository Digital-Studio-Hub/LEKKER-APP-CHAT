...
- `server/routes.ts` - API endpoints:
  ...
  - `/api/auth/forgot-password` - Request password reset code (rate-limited, 6-digit code, 15min expiry)
  - `/api/auth/verify-reset-code` - Verify reset code validity (rate-limited)
  - `/api/auth/reset-password` - Reset password with valid code (rate-limited, enforces password strength)
