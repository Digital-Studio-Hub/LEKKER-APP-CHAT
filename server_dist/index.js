...
var phoneVerificationCodes = pgTable("phone_verification_codes", {
  id: varchar("id", { length: 36 }).primaryKey().default(sql`gen_random_uuid()`),
...
var passwordSchema = z.string().min(8, "Password must be at least 8 characters").regex(/[A-Z]/, "Password must contain at least one uppercase letter").regex(/[0-9]/, "Password must contain at least one number").regex(/[^A-Za-z0-9]/, "Password must contain at least one special character");
...
// server/routes.ts
import { sql as sql3, or as or2, and as and2, ne as ne2, eq as eq2 } from "drizzle-orm";
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/auth/send-email-code", phoneVerifyLimiter, async (req, res) => {
    try {
      ...
      await db.delete(emailVerificationCodes).where(eq2(emailVerificationCodes.email, email.trim().toLowerCase()));
      const code = Math.floor(1e5 + Math.random() * 9e5).toString();
      const expiresAt = new Date(Date.now() + 10 * 60 * 1e3);
  ...
  app2.post("/api/auth/verify-email-code", phoneVerifyLimiter, async (req, res) => {
    try {
      ...
      const [record] = await db.select().from(emailVerificationCodes).where(eq2(emailVerificationCodes.email, email.trim().toLowerCase())).orderBy(emailVerificationCodes.createdAt).limit(1);
      if (!record) {
      ...
      await db.update(emailVerificationCodes).set({ verified: true }).where(eq2(emailVerificationCodes.id, record.id));
  ...
  app2.post("/api/auth/verify-phone-code", phoneVerifyLimiter, async (req, res) => {
    try {
      ...
      await db.update(phoneVerificationCodes).set({ verified: true }).where(eq2(phoneVerificationCodes.id, record.id));
  ...
  app2.post("/api/auth/verify-reset-code", resetRequestLimiter, async (req, res) => {
    try {
      ...
      const resetCodes = await db.select().from(passwordResetCodes).where(
        and2(
          eq2(passwordResetCodes.email, resolvedEmail),
          eq2(passwordResetCodes.code, code.trim()),
          eq2(passwordResetCodes.used, false)
        )
      ).orderBy(sql3`created_at DESC`).limit(1);
  ...
  app2.post("/api/auth/reset-password", resetRequestLimiter, async (req, res) => {
    try {
      ...
      await db.update(passwordResetCodes).set({ used: true }).where(eq2(passwordResetCodes.id, resetCode.id));
    ...
    try {
      ...
      if (chatType === "p2p") {
        ...
        if (existing) {
          ...
          return res.json({ chat: { ...existing, participants: participants2 } });
        }
  ...
  app2.put("/api/chats/:chatId/messages/:messageId", authMiddleware, async (req, res) => {
    try {
      ...
      const [msg] = await db.select().from(chatMessages).where(
        and2(eq2(chatMessages.id, messageId), eq2(chatMessages.chatId, chatId))
      ).limit(1);
      if (!msg) {
      ...
      const [updated] = await db.update(chatMessages).set({ content: content.trim(), editedAt: /* @__PURE__ */ new Date() }).where(eq2(chatMessages.id, messageId)).returning();
  ...
  app2.delete("/api/chats/:chatId/messages/:messageId", authMiddleware, async (req, res) => {
    try {
      ...
      const [updated] = await db.update(chatMessages).set({ isDeleted: true, content: null, imageUri: null, fileUri: null, audioUri: null }).where(eq2(chatMessages.id, messageId)).returning();
      res.json({ message: updated });
    } catch (error) {
    ...
    try {
      ...
      const results = await db.select({
        ...
        and2(
          ne2(users.id, userId),
          or2(
            sql3`LOWER(${users.firstName}) LIKE ${`%${query}%`}`,
